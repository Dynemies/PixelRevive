...
Removes multiplicative speckle and additive Gaussian noise and performs 2× super-resolution in a single forward pass. 4.02 M parameters.
...
That is all. The script loads checkpoints/best.pt itself — nothing to configure, download or edit. It works from any working directory, uses a GPU if one is present and the CPU otherwise, and needs no network access.
...
Input: .npy, .png, .tif, .jpg. Output: one restored image per input, same filename, same format. A 128×128 input gives a 256×256 output.
...
+5.57 dB over bicubic, +1.48 dB over the strongest classical baseline. Over all 384 validation images the model reaches 28.50 dB.

Validation is held out by scene: the 3,200 training images contain only 2,589 distinct scenes, so a random split would leak overlapping crops.
...
src/train.py	Reproduces training from scratch (~16.6 h on one T4)
...
python src/train.py                                  # ~16.6 h on one Tesla T4
Trained on one NVIDIA Tesla T4 (16 GB), Kaggle free tier, 16.6 hours, at no cost.

Approach in one paragraph
We recovered the degradation model from 3,200 training pairs before choosing an architecture: the speckle is Gamma with shape 11–55, the downsampling is separable bicubic with a = −0.75, and the Gaussian noise is applied last. The model is a four-stage unrolled solver whose learned denoiser is a NAFNet U-Net shared across stages, conditioned on noise parameters predicted from the input. Loss is Charbonnier plus a frequency-domain term; no adversarial loss, which would lower PSNR and invent detail that was never measured. Full detail, and the ablations showing which of our design choices actually helped, are in docs/METHOD.md.
